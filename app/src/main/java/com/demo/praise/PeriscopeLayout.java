...
public class PeriscopeLayout extends RelativeLayout {
    ...
    private List<ImageView> heartViews;
    ...
    private void init() {
        ...
        Drawable red = getResources().getDrawable(R.drawable.ytm_taobao_zan1);
        Drawable yellow = getResources().getDrawable(R.drawable.ytm_taobao_zan2);
        Drawable blue = getResources().getDrawable(R.drawable.ytm_taobao_zan3);
    ...
    public void addHeart() {
        if (heartViews.size() == 0) {
            heartView = new ImageView(getContext());
        } else {
            synchronized (heartViews) {
                heartView = heartViews.remove(0);
            }
        }
        //随机选一个
        heartView.setImageDrawable(drawables[random.nextInt(3)]);
        heartView.setLayoutParams(lp);

        addView(heartView);

        Animator set = getAnimator(heartView);
        set.addListener(new AnimEndListener(heartView));
        set.start();
    ...
    private class AnimEndListener extends AnimatorListenerAdapter {
        ...
        public void onAnimationEnd(Animator animation) {
            ...
            target.setX(getWidth() / 2);
            target.setY(getHeight());
            synchronized (heartViews) {
                heartViews.add(target);
            }
        }
    }
